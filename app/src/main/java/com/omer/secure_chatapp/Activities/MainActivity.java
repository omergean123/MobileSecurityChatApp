...
import com.omer.secure_chatapp.Adapters.RecentConversationAdapter;
...
import com.omer.secure_chatapp.databinding.ActivityMainBinding;
...
public class MainActivity extends BaseActivity implements ConversationListener {
    ...
    private List<ChatMessage> convs;
    private RecentConversationAdapter convAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getToken();
        setListeners();
        listenerConverstaions();
        getkeys();
    }
    private void init(){
        convs = new ArrayList<>();
        convAdapter = new RecentConversationAdapter(convs,this);
        binding.conversationsRecycleView.setAdapter(convAdapter);
    ...
    private void listenerConverstaions(){
            ...
            for(DocumentChange documentChange : value.getDocumentChanges()){
                if(documentChange.getType() == DocumentChange.Type.ADDED){
                    ...
                    if(preferenceManager.getString(Constants.KEY_USER_ID).equals(senderId)){
                        ...
                        chatMessage.setConvertsationId(documentChange.getDocument().getString(Constants.KEY_RECEIVER_ID));
                    }else{
                        ...
                        chatMessage.setConvertsationId(documentChange.getDocument().getString(Constants.KEY_SENDER_ID));
                    }


                    chatMessage.setMessage(documentChange.getDocument().getString(Constants.KEY_LAST_MESSAGE)); //dec
                    chatMessage.setDate(documentChange.getDocument().getDate(Constants.KEY_TIMESTAMP));
                    convs.add(chatMessage);
                }else if(documentChange.getType() == DocumentChange.Type.MODIFIED){
                    for (int i = 0; i< convs.size(); i++){
                        String senderId = documentChange.getDocument().getString(Constants.KEY_SENDER_ID);
                        String receiverId = documentChange.getDocument().getString(Constants.KEY_RECEIVER_ID);
                        if(convs.get(i).getSenderId().equals(senderId) && convs.get(i).getReceiverId().equals(receiverId)){
                            convs.get(i).setMessage(documentChange.getDocument().getString(Constants.KEY_LAST_MESSAGE));
                            convs.get(i).setDate(documentChange.getDocument().getDate(Constants.KEY_TIMESTAMP));
                            break;
                        }
                    }
                }
            }
            Collections.sort(convs,(obj1,obj2) -> obj2.getDate().compareTo(obj1.getDate()));
    ...
    private void updateToken(String token){
        ...
        documentReference.update(Constants.KEY_FCM_TOKEN,token)
                .addOnFailureListener(e->showToasts("Unable to update token"));
    }
    ...
    @Override
    public void onConversationClickd(User user) {
